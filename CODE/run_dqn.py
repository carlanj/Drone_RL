...
class DroneEnv(gym.Env):
    ...
    def __init__(self, lidar_name="Lidar1", vehicle_name="Drone1", debug=False, render_mode=None):
        super().__init__()
        self.vehicle_name = vehicle_name
        self.lidar_name   = lidar_name
        self.debug = debug
        self.render_mode = render_mode  # keep API compatibility

        # Discrete action space for DQN
        self.action_space = spaces.Discrete(7)  # forward, left, right, up, down, yawL, yawR
...
class DQNTrainer:
    def __init__(self, log_dir="dqn_runs/default"):
        os.makedirs(log_dir, exist_ok=True)
        env = DroneEnv()
        self.env = DummyVecEnv([lambda: Monitor(env, log_dir)])
        self.folder = log_dir
        ...
        self.checkpoint_callback = CheckpointCallback(
            save_freq=10_000, save_path=self.checkpoint_dir, name_prefix="checkpoint"
        )
        self.eval_env = DummyVecEnv([lambda: DroneEnv()])
        self.eval_callback = EvalCallback(
            self.eval_env, best_model_save_path=self.best_model_dir,
            log_path=self.eval_log_dir, eval_freq=5_000,
            deterministic=True, render=False
        )
        self.train_logger = TrainLogger(folder=self.folder)
    ...
    def train(self, steps=200_000):
        model = DQN("MlpPolicy", self.env, verbose=1,
                    tensorboard_log=self.folder,
                    buffer_size=50_000, learning_rate=1e-3,
                    batch_size=64, train_freq=4, gamma=0.99,
                    target_update_interval=500, exploration_fraction=0.1,
                    exploration_final_eps=0.05)
        model.learn(total_timesteps=steps, callback=self.callback)
        final_path = os.path.join(self.folder, "final_model")
        model.save(final_path)
        print(f"✅ DQN training complete. Saved to {final_path}")
# ================= Continuation Trainer =================
class DQNCont:
    def __init__(self, model_path, steps=100_000,
                 out_folder="dqn/static_runs/cont", log_every="episode"):
        ...
        env = DroneEnv()
        self.env = DummyVecEnv([lambda: Monitor(env, self.out_folder)])
        self.log_cb = TrainLogger(folder=self.out_folder, tag=log_every)

    def run(self):
        model = DQN.load(self.model_path, env=self.env,
                         tensorboard_log=self.out_folder, verbose=1)
        ...
        self.train_logger = TrainLogger(folder=self.out_folder)

        self.callback = CallbackList([self.checkpoint_callback, self.eval_callback, self.train_logger])

        model.learn(total_timesteps=self.steps, callback=self.callback)
...
class DQNEval:
    def __init__(self, model_path, episodes=5,
        ...
        env = DroneEnv(vehicle_name=vehicle_name, lidar_name=lidar_name)
        self.env = DummyVecEnv([lambda: env])
        self.model = DQN.load(self.model_path, env=self.env)
        self.log_cb = TrainLogger(folder=self.out_dir, tag=log_every)

    def run(self):
        env = DroneEnv(vehicle_name=self.vehicle_name, lidar_name=self.lidar_name)
        ...
        for ep in range(self.episodes):
            ...
            total_reward = 0.0
            while not done:
                # SB3 expects obs shaped like (n_envs, obs_dim) when using predict
...
if __name__ == "__main__":
    # Train from scratch
    trainer = DQNTrainer(log_dir="DQN/static_runs_advanced/150k")
    trainer.train(steps=150_000)

   # 2) Continue training
    # dqn_cont = DQNCont(model_path="dqn\\static_runs\\350k\\continues\\30k\\checkpoints\\checkpoint_30000_steps",
    #                    steps=1000, out_folder="dqn/static_runs/350k/continues/30k/cont")
    # dqn_cont.run()

    # # 3) Evaluate
    # dqn_eval = DQNEval(model_path="C:\\THESIS\\dqn\\static_runs\\350k\\continues\\30k\\best_model\\best_model",
    #                    episodes=3, out_dir="C:\\THESIS\\dqn\\static_runs\\350k\\continues\\30k\\evaluations\\best")
    # dqn_eval.run()